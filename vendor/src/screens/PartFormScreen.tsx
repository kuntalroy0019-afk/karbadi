...
export default function PartFormScreen({ route }: any) {
  ...
  useEffect(() => {
    ...
    if (editId) {
      Catalog.part(editId).then((r) => {
        const p = r.data;
        setForm({
          title: p.title, description: p.description || "", category: p.category ?? 0,
        ...
        setExisting((p.images || []).map((i) => i.image));
  ...
  async function pick() {
    ...
    const res = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ["images"], quality: 0.7, allowsMultipleSelection: true, selectionLimit: 5 });
    if (res.canceled) return;
    setImages((cur) => [...cur, ...res.assets.map((a, i) => ({
      uri: a.uri, name: a.fileName || `photo_${i}.jpg`, type: a.mimeType || "image/jpeg",
    }))]);
  }

  async function save() {
    if (!form.title.trim() || !form.price || !form.category)
      return Alert.alert("Missing fields", "Title, category and price are required.");
    setSaving(true);
    const d = new FormData();
    d.append("title", form.title); d.append("description", form.description);
    d.append("category", String(form.category)); d.append("condition", form.condition);
    d.append("oem_part_number", form.oem_part_number); d.append("price", form.price);
    if (form.mrp) d.append("mrp", form.mrp);
    d.append("stock", form.stock || "1"); d.append("compatible_vehicles", form.compatible_vehicles);
    images.forEach((img) => d.append("uploaded_images", img as any));
    try {
      if (editId) await Catalog.updatePart(editId, d); else await Catalog.createPart(d);
      Alert.alert("Saved", `Listing ${editId ? "updated" : "published"}.`);
      nav.goBack();
  ...
  return (
    <ScrollView style={{ backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl }}>
      ...
      <View style={styles.row}>
        ...
        <View style={{ width: 84 }}><Label text="Stock" /><Input value={form.stock} onChangeText={set("stock")} keyboardType="numeric" /></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  remove: { position: "absolute", top: -4, right: -4, backgroundColor: colors.danger, width: 20, height: 20, borderRadius: 10, alignItems: "center", justifyContent: "center" },
